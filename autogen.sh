#!/bin/sh
#
# this will run swig in the needed directorys and 
...
function NeedToSWIG()
{
	...
	#### /-- checks
	if ( ( [ ! -f "OgreNet/OgreBindings_wrap.cxx" ] ) || ( [ ! -f "OgreNet/OgreBindings.cs" ] ) || ( [ ! -f "OgreNet/OgreBindings_Compositor_wrap.cxx" ] ) || ( [ ! -f "OgreNet/OgreBindings_Compositor.cs" ] ) ); then
		runswigOGRE='yes'
	else
